...
public static class JsonManager
{
	...
	private static void OnFileChanged(object sender, FileSystemEventArgs e)
	{
		...
		if (_outputToSource.TryGetValue(fullPath, out string? sourcePath))
		{
			try
			{
				File.Copy(fullPath, sourcePath, overwrite: true);
			}
			catch
			{
			}

			if (_files.TryGetValue(sourcePath, out JsonFileData? sourceData))
			{
				sourceData.NeedsReload = true;
			}
		}
	}
	...
	private sealed class JsonFileData
	{
		...
		public void Reload()
		{
			...
			for (int attempt = 0; attempt < 3; attempt++)
			{
				try
				{
					...
					Data = JsonSerializer.Deserialize<Dictionary<string, object>>(json, _readOptions);
					NeedsReload = false;
					IsDeleted = false;
				...
				{
					System.Diagnostics.Debug.WriteLine($"[JsonManager] Failed to load {FilePath}: {ex.Message}");
					return;
				}
			}
		}
		...
		public void Save()
		{
			...
			{
				System.Diagnostics.Debug.WriteLine($"[JsonManager] Failed to save {FilePath}: {ex.Message}");
			}
			finally
			{
