...
- **`Reset.all()` also invokes any module-exposed `reset()` function** (after the array wipe). Modules owning non-array resettable state implement `reset()` so a restart restores neutral state — e.g. `DayCycle.reset()` restores `time`/`display` (a number/dict the array-only wipe skips), which drive the DayNightGrade shader uniform and the shadow every frame, so they would otherwise freeze at the pre-restart values. DEV RULE: a `reset()` must not read other modules' state (iteration order over `package.loaded` is non-deterministic), and any file-scope local state must be reset explicitly inside `reset()` (Reset.all only sees module-table fields, not upvalues).
...
- **The death anim plays through a frozen world.** On DEATH, Main sets `state = "dying"` (`simulating` gates off the world loops) and emits `STATE_CHANGED("death")` — the event-driven way to force an anim without violating Control's single-writer rule (Control never runs again, so the anim sticks). Because the sprite update loop is frozen, Main advances ONLY the player's spritesheet (and shake) manually each frame with raw `dt` — never `playerSprite:update(dt)`, which would re-run Control and rewrite `_state`. The anim's last `ANIM_FRAME` (frameIndex ≥ death anim `frames`) flips `state = "gameover"` for the hold.
- **Death reveal expands the CircleMask back open, then the canvas fades to black via the `Darken` post-process shader.** On DEATH Main snaps zoom to `lowSatietyZoom`, then eases zoom/saturation/posterize/circle back to normal, setting `current == target` each frame so `Zoom.update` and the circle ease see a no-op and don't fight the reveal. It then fades `u_darken` 0→1 to black the whole canvas — NOT by closing the circle. The game also fades in from black at start (`u_darken` 1→0).
- **LuaJIT caps a function at 60 upvalues; `love.update` hits it quickly.** Any logic that would add locals/closures to `love.update` must live in its own function (own upvalue set). `updateHold`, `updateReveal` and `updateStartDarken` were extracted for exactly this reason. Symptom: `Syntax error: function ... has more than 60 upvalues`. The cursor update was moved into `love.draw` (wall-clock `dt` via `love.timer.getTime()`) rather than `love.update` for this reason — `cursorSprite` is already an upvalue of `love.draw`, so no new upvalue is added there either.
- **A sprite data file with `components` non-empty skips the StaticSprite gate in `SpriteLoader.instantiate`.** `sprite.type` is only set to `"StaticSprite"` when `data.components` is empty; `Sprite:draw()` only renders a sprite with `type == "StaticSprite"` and a loaded `image`. A plain-PNG sprite that needs non-visual components (e.g. the cursor: `tween` + `cursor`) therefore renders nothing unless you force `sprite.type = "StaticSprite"` after `instantiate` (done in `main.lua` for the cursor). Don't add a `spritesheet` just to satisfy the gate, and don't change the gate in `SpriteLoader` — the override at the call site is the intended fix.
- **`Tools/LuaFormatter/Formatter.py` absorbs a `-- comment` line into the following code line.** A comment directly above a data value (`stepInterval = 2,`) gets merged so the value ends up inside the comment and is silently dropped — the death-sound `stepInterval` guard vanished this way and the death sound replayed on frames. Do not put a `--` comment on its own line immediately before a config value you need to keep; verify the formatted output.
- **Freeze a terminal state (death) by gating the world-simulation loops on `state == "game"`, not per-component flags.** On `"dying"`/`"gameover"`, Main computes `simulating = state == "game"` and wraps the destruction/drop/tween processing, the dynamic-object update loop, camera/attack, and streaming/`PropSpawner` in it — so props stop breaking (no explosion/drop/sound) and pickups stop following (no pickup pling). Leave UNGATED whatever the state must keep running: the manual player-spritesheet advance, `ShaderLoader.update`, `Zoom.update`, UI. `love.audio.stop()` only silences currently-playing sources — the gates stop new ones.
