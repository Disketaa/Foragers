...
- **The death screen keeps the CircleMask at its satiety-0 radius — no blackout.** The circle is NOT shrunk further at death; it just holds whatever radius satiety 0 set (the small 48px hole showing the dead player). Do not reintroduce a mask close or a black-clear canvas on the death path. Red flash, death burst particle and `love.audio.stop()` were removed from the death path.
