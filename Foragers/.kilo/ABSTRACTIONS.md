...
- **Screen post-process = auto-composed modules, not a program data file.** Any
  ...
  post-process. **The death blackout does NOT use this switch anymore** — it keeps
  the mask on and closes `u_circleRadius` to `DEATH_CIRCLE_MIN` so the CircleMask
  visibly swallows the frozen world. Never add `isDead and nil or getPostProcess()`
  at the call site.
...
- **The death anim plays through a frozen world.** On DEATH, Main sets `state = "dying"` (`simulating` gates off the world loops) and emits `STATE_CHANGED("death")` — the event-driven way to force an anim without violating Control's single-writer rule (Control never runs again, so the anim sticks). Because the sprite update loop is frozen, Main advances ONLY the player's spritesheet manually each frame (`ss:update(dt)`), never `playerSprite:update(dt)` — that would re-run Control and rewrite `_state`. The anim's last `ANIM_FRAME` (frameIndex ≥ death anim `frames`) flips `state = "gameover"` and starts the blackout.
- **The death blackout is the CircleMask closing, not a black canvas.** At `"gameover"` the frozen world keeps drawing and the mask radius eases to `DEATH_CIRCLE_MIN = 0.00001` (NOT 0 — the CircleMask shader early-returns at `u_circleRadius <= 0`, which would clear the mask entirely), so the circle visibly swallows the world. `circleMaskSmoothness` is bumped for the blackout and restored in `initGame`. Red flash, death burst particle and `love.audio.stop()` were removed from the death path.
