...
| **emissive** | Marker for emissive render pass. Sprites with this component are rendered to an offscreen canvas at native 1x, then blitted to screen at integer scale — two-stage split avoids compounded fractional scale that breaks nearest-neighbor on direct-to-screen draw. No shader UV sampling; canvas blit handles pixel alignment. | — | — | — |
