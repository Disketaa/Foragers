...
- **Entry:** `CardSelect.start(uiSprites)` filters cards by `cardAvailable()` (respects `maxLevel` per group), then limits the shown pool to `MAX_VISIBLE_CARDS` using rarity-weighted reservoir sampling (weights from `Source/Helpers/Systems/Rarities.lua`, data in `Content/Data/Rarities.lua`). Each picked card is assigned a rarity via `Rarities.pick()` and its palette is updated to match. Lays out centered, sets `GameState.state = "cardselect"`, starts the selection darken via `PostProcess.startSelectionDarken()`, and creates a `GridNav` for keyboard/mouse navigation.
