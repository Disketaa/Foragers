...
| **silhouette** | Marker for silhouette reveal effect. `mode="silhouette"` (default) — player/items captured to canvas. `mode="mask"` — foliage reveals silhouettes where its alpha > 0. Visually inert — canvas rendered by `Helpers/Mask.lua`, reveal comes from `"Silhouette"` shader module on the tree | `mode` (`"silhouette"` default, `"mask"` for trees) | — | — |
...
| **player_stats** | Player stats container (crit, level, xp, satiety) | `critChance`, `critMult`, `level`, `experience`, `xpCurve`, `satiety`, `maxSatiety` | — | VALUE_CHANGED |
