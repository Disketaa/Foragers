...
| **shadow** | Texture-free pixel-perfect shadow (3-rect 1px-rounded shape). Length is driven globally by DayCycle (`dayCycle.shadow.stretchPx`); the sun appends pixels to the far edge while the near edge stays anchored under the character, so the shadow lengthens without translating off the character | `offsetX`, `offsetY`, `width`, `height`; `offsetMultiplier` (reserved — unused since the sun no longer translates the shadow) | — (data-only; rendered via `Shadow.renderLayer`) | — |
