...
|---|---|---|---|---|
| **spritesheet** | Quad animation + frame rendering. Each animation accepts per-frame `duration` hold weights (frame `i` holds `duration[i]/speed`; see data-format.md). Exposes `getAnimFrameIndex()` (1-based current anim frame) and `setAnimFrameIndex(index)` (snap to a frame, continuing to animate from there) — used by Overlay `inheritFrame` to spawn a child on the host's current frame | `columns`, `rows`, `animations`, `tags` | STATE_CHANGED (5) | ANIM_FRAME |
...
| Component | Purpose | Config | Subscribes | Emits |
|---|---|---|---|---|
| **tween** | Property animation on events (flip, hit, state, arrival, spawned, pickup, counter tick/wrap). Also exposes `triggerTag(name)` for externally-driven tweens with no event (used by `cursor` to fire `show`/`hide`). | `tweens`, `tags` (event→tween mappings), `destroyOnComplete` | FLIPPED (10), STATE_CHANGED (10), PROP_HIT (10), FOLLOW_ARRIVED (10), PROP_SPAWNED (10), PICKUP (10), COUNTER_TICK (10), COUNTER_WRAP (10) | TWEEN_COMPLETED |
...
| **shadow** | Texture-free pixel-perfect shadow (3-rect 1px-rounded shape). During golden hours (sunrise/sunset) the shadow stretches right (sunrise) / left (sunset) via `dayCycle.shadow.stretchPx`; the near edge stays anchored under the character so it lengthens without translating off. The golden-hour stretch/offset/alpha is **phase-shifted per prop by world X** (`dayCycle.shadow.timeShiftPerPx`, `worldCenterX`): each prop reads `DayCycle.getSunData(effTime)` at its own eased clock, so the sweep crosses the island instead of all props peaking at once. Per-prop alpha is baked into the spritebatch color; the canvas composites at a constant `layerAlpha`. | `offsetX`, `offsetY`, `width`, `height`; `offsetMultiplier` (reserved — unused since the sun no longer translates the shadow) | — (data-only; rendered via `Shadow.renderLayer`) | — |
...
| **ambient** | Freely-drifting visual sprite (butterflies, fireflies) with lifetime-based fade-out and day/night despawn. No collision, no destructible, no shadow — pure visual ambient layer. Wander direction changes every `interval` seconds. Fades alpha over `fadeOutDuration` then marks despawned. Day/night gating subscribes to `TIME_CHANGED` on `DayCycle.emitter` (NOT parent sprite) | `wanderingSpeed`, `duration` (lifetime seconds), `fadeOutDuration`, `interval` (direction change), `despawnOnNight` (bool), `despawnOnDay` (bool) | `TIME_CHANGED` (100, on `DayCycle.emitter`) | — |
| **palette** | Applies palette colors (tier or rarity) to the parent sprite. Resolves name from `level` via `Tiers.tierNameForLevel()` or a local `rarityNameForLevel()` helper, then writes `u_tier_1..u_tier_5` uniforms to the parent's `shaderData` and the parent's `shader` component. For `scheme="rarity"`, also sets `u_tier_5` from the last rarity color for the lum < 0.65 shader fallback. Invalidates all `image` component canvases when `scheme="tier"`. | `scheme` (`"tier"` or `"rarity"`), `tier` (string, for tier scheme), `rarity` (string, for rarity scheme), `level` (number, optional — resolves name from level) | — | — |
| **emissive** | Marker for emissive render pass. `attach()` sets `parent._hasEmissive = true`; Main skips these sprites in the world draw loop and renders them between the pre-shader (DayNightGrade) and post-shader (Saturation+CircleMask) passes. Sprites are drawn to an offscreen canvas at native 1x, then blitted to screen at integer scale — two-stage split avoids compounded fractional scale that breaks nearest-neighbor on direct-to-screen draw. No shader UV sampling; canvas blit handles pixel alignment. `drawToScreen` accepts an optional `targetCanvas` to composite into an intermediate canvas instead of screen. | — | — | — |
...
- `fpsGraph` — inline line graph of recent FPS samples (60-point ring buffer), drawn to the right of the FPS text on the same line, `gap` px after it. The reference line is `Options.maxFps` (the frame cap); bar height scales to it. Segments are green (`goodColor`) while at/above the real ceiling, red (`badColor`) below it — the ceiling is `min(maxFps, displayRefreshRate)` (so a 144hz panel isn't painted red against an unreachable 180 target), and `tolerance` is a **fraction** of that ceiling (0.5 = 50%, default 0) below which a segment goes red (e.g. 0.5 at 60fps → red below 30). A dim `labelColor` marker is drawn at the ceiling when it's below `maxFps`. `goodColor`/`badColor` live at the `hud` level and are shared with the `toggles` readout. `width`/`height` set the graph box size (clamped to the row) and `thickness` the line weight, all in base px. The FPS value text is right-aligned into a fixed width (from `maxFps` digit count) so the graph never shifts when the reading goes 2→3 digits.
...
- `profiler` — auto-instrumenting CPU profiler drawn as a bottom-left table (`Scope`/`Time`/`%`) in the same styling. It needs no imports: it patches `Sprite:addComponent` to time every component's `update`/`draw`, and wraps any module exposing a known per-frame method name (`update`, `updateAll`, `updateBursts`, `updateDetached`, `drawAll`, ...) via a `package.loaded` sweep plus a `require` wrapper, so new system files are timed automatically. `updateSpeed` (Hz) controls the flush cadence; `limit` caps rows (sorted by cost); `nameMaxChars` truncates scope names; `digits` sets time decimal precision and `valueMaxChars` caps the time column width (which is fixed so a digit-boundary change never shifts the `%` column). Scope keys are per component type (`ComponentType.update`). The table renders only while `hud` is on (F1 hides it); collection keeps running while hidden so data stays fresh.
- `snapshot` (top-level `Content/Data/Debug.lua` group, NOT under `hud`) — FPS-dip + ambient diagnostics logger to `Logs/Snapshot.txt`. While the profiler collects, it records each FPS dip below `fpsTarget` (falls back to `hud.fpsGraph.fpsTarget`, then 60), capturing the top `topScopes` profiler scopes plus an ambient line (frame split into update/draw/idle, measured vs unmeasured, GC, drawcalls, window). `rollupFps` (Hz; 0 = off) appends a sparse steady-state ambient line even when FPS never dips. Reads `debugData.snapshot`; implemented in `Source/Helpers/Debug/Snapshot.lua`, driven by `Snapshot.captureDraw`/`mark*Start`/`set*End` called from Main/Debug.

Implemented in `Source/Helpers/Debug/Debug.lua` (the shared Debug helper, one file): `Debug.update(dt)` samples FPS into a ring buffer and flushes profiler buckets; `Debug.draw(objectCount, scale)` renders the readout using LÖVE's default font (recreated when the scaled `size` changes). `Debug.enabled`/`Debug.settings` resolve dotted group paths, so the graph sub-group is queried as `"hud.fpsGraph"` and the profiler as `"hud.profiler"`. `Debug.showing(group)` returns `enabled("hud") and enabled(group)` and is the shared gate for all overlay rendering.

## Debug chat

The `hud.chat` sub-group renders a bottom-left console: a text-input row with a blinking I-beam while open, plus (after submitting a command) a single `Unknown command: "..."` output line in `badColor` that auto-hides after `outputTimeout` seconds. Toggled with the `toggleChat` keybind (default Enter); the console captures the keyboard so gameplay input is ignored while typing.
...
`Source/UI/CardSelect.lua` drives the level-up card picker. It is not a component — it is a global system called from `main.lua`.

- **State:** `GameState.state = "cardselect"` pauses the world (`simulating` gate in Main). `GameState.showingCards = true` while the picker is open.
- **Entry:** `CardSelect.start(uiSprites)` loads card definitions once via `SpriteLoader.loadDefs()` (cached in `_cardDefs`), filters by `cardAvailable()` (respects `maxLevel` per group), then limits the shown pool to `MAX_VISIBLE_CARDS` using rarity-weighted reservoir sampling (weights from `Source/Helpers/Systems/Rarities.lua`, data in `Content/Data/Rarities.lua`). Each picked card is assigned a rarity via `Rarities.pick()` and its palette is updated to match. Cards are instantiated on-demand via `SpriteLoader.instantiate(freshData(def.data), ...)` — only the 3 picked cards are spawned, not all possible cards. Lays out centered, sets `GameState.state = "cardselect"`, starts the selection darken via `PostProcess.startSelectionDarken()`, and creates a `GridNav` for keyboard/mouse navigation.
- **Repeat on multiple levels:** `GameState.pendingLevelUps` is incremented on `LEVEL_UP` (by `PlayerStats`) and decremented when a card is chosen. While `> 0`, Main re-shows cards after each pick. `CardSelect.shouldShow(uiSprites)` returns true if any card is still below its `maxLevel`; the caller consumes the pending level-up regardless so a fully-maxed run never retries.
- **Pick:** `onConfirm` calls `CardSelect.applyModifier(sprite)` (applies `modifier` function or `{stat, amount}` table, increments `GameState.cardGroupCounts[grp]`, refreshes weapon UI level text/emblem/palette if the pickaxe group changed), adds `ZOOM_ADD` to zoom, then `CardSelect.hide(sprite)`.
- **Hide:** `CardSelect.hide(chosenSprite)` plays `chosen` or `hide` tween tags, emits `CARD_CHOOSE` or `CARD_HIDE`, sets `GameState.state = "game"` immediately so the world runs underneath the chosen card's burn animation, and clears `showingCards` once all tweens finish in `update()`.
- **Events emitted:** `CARD_SELECT_OPEN` (first card shown), `CARD_SELECTED` (highlight moves), `CARD_HIDE` (non-chosen cards), `CARD_CHOOSE` (chosen card). `Sound` subscribes to all four at priority 15.
