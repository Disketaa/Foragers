...
-- Lua 5.1 compatibility: love.handlers hijacks global unpack
-- Provide table.unpack if it doesn't exist (Lua 5.1/LuaJIT)
if not table.unpack then
    local function _lu51_unpack(t, i, j)
        i = i or 1
        j = j or #t
        if i > j then return end
        return t[i], _lu51_unpack(t, i + 1, j)
    end
    table.unpack = _lu51_unpack
end
...
function ____exports.loadLuaString(str, env)
    if _G.setfenv ~= nil then
        local f, e = _G.loadstring(str, str)
        if f and env then
            _G.setfenv(f, env)
        end
        return f, e
...
do
    ...
    function Path.dirName(path)
        local dir = string.match(path, ((("^(.-)" .. Path.separator) .. "+[^") .. Path.separator) .. "]+$")
        return dir or "."
    end
    function Path.splitDrive(path)
        local drive, pathPart = string.match(path, "^[@=]?([a-zA-Z]:)[\\/](.*)")
        if drive then
            drive = string.upper(drive) .. Path.separator
        else
    ...
    function Path.format(path)
        ...
        if not formattedPath then
            ...
            local formattedDrive = string.gsub(drive, "[\\/]+", Path.separator)
...
do
    ...
    local function base64Decode(input)
        ...
        for c in string.gmatch(input, ".") do
            ...
            for i = 1, 6 do
                local ____bit = sextet % 2 ~= 0
                table.insert(bits, i, ____bit)
                sextet = math.floor(sextet / 2)
            end
            if #bits >= 8 then
                local value = 0
                for i = 7, 0, -1 do
                    local ____bit = table.remove(bits)
                    if ____bit then
                        value = value + 2 ^ i
                    end
                end
    ...
    local function build(data, mapDir, luaScript)
        local sources = string.match(data, "\"sources\"%s*:%s*(%b[])")
        local mappings = string.match(data, "\"mappings\"%s*:%s*\"([^\"]+)\"")
        ...
        local sourceRoot = string.match(data, "\"sourceRoot\"%s*:%s*\"([^\"]+)\"")
        ...
        for source in string.gmatch(sources, "\"([^\"]+)\"") do
            if Path.isAbsolute(source) then
                ...
                local sourcePath = (((mapDir .. Path.separator) .. sourceRoot) .. Path.separator) .. tostring(source)
                table.insert(
                    sourceMap.sources,
        ...
        local names = string.match(data, "\"names\"%s*:%s*(%b[])")
        local nameList
        if names then
            nameList = {}
        ...
        local nameIndex = 0
        for mapping, separator in string.gmatch(mappings, "([^;,]*)([;,]?)") do
            if #mapping > 0 then
                local colOffset, sourceOffset, sourceLineOffset, sourceColOffset, nameOffset = table.unpack(
                ...
                if nameList and nameOffset then
                    ...
                    if not luaLines then
                        luaLines = {}
                        for luaLineStr in string.gmatch(luaScript, "([^\r\n]*)\r?\n") do
                            table.insert(luaLines, luaLineStr)
                        end
                    end
                    local luaLine = luaLines[line]
                    if luaLine then
                        local luaName = string.match(
                            string.sub(luaLine, column),
                            "[a-zA-Z_][A-Za-z0-9_]*"
                        )
                        if luaName then
                ...
                if not lineMapping or sourceLine < lineMapping.sourceLine and column < lineMapping.column or sourceLine == lineMapping.sourceLine and sourceColumn < lineMapping.sourceColumn then
                    sourceMap.mappings[line] = {sourceIndex = sourceIndex, sourceLine = sourceLine, sourceColumn = sourceColumn, column = column}
                end
            end
            if separator == ";" then
                line = line + 1
    ...
    local function getScriptRoots()
        if not scriptRoots then
            ...
            if scriptRootsStr then
                for path in string.gmatch(scriptRootsStr, "[^;]+") do
                    local formattedPath = Path.format(path) .. Path.separator
    ...
    local function getMap(filePath, file)
        ...
        local encodedMap = string.match(data, "--# sourceMappingURL=data:application/json;base64,([A-Za-z0-9+/=]+)%s*$")
        if encodedMap then
            ...
            if sourceMap ~= false then
                for ____, source in ipairs(sourceMap.sources) do
                    if source == sourceFile then
                        return tostring(scriptFile), sourceMap
                    end
                end
            end
        end
...
do
    ...
    local function escape(str)
        local escaped = string.gsub(str, escapesPattern, replaceEscape)
        return escaped
    end
...
do
    ...
    function Send.vars(varsObj)
        ...
        for name, info in pairs(varsObj) do
            local dbgVar = name == "..." and buildVarArgs(name, info.val) or buildVariable(
                tostring(name),
                info.val
            )
    ...
    function Send.help(...)
        ...
        local builtStrs = {}
        for _, nameAndDesc in ipairs(helpStrs) do
            local name, desc = table.unpack(nameAndDesc)
            table.insert(
                builtStrs,
...
local luaTable = _G.table
if not luaTable.unpack then
    luaTable.unpack = _G.unpack
end
...
do
    ...
    local function isValidIdentifier(name)
        if supportsUtf8Identifiers then
            for c in string.gmatch(name, "[^a-zA-Z0-9_]") do
            ...
            local invalidChar = string.match(name, "[^a-zA-Z0-9_]")
    ...
    local function mapExpressionNames(expression, sourceMap)
        ...
        for i = 1, #expression do
            ...
            elseif char == "\"" or char == "'" then
                ...
                local nameChar = string.match(char, "[^\"'`~!@#%%^&*%(%)%-+=%[%]{}|\\/<>,%.:;%s]")
                if nameStart then
                    if not nameChar then
                        local sourceName = string.sub(expression, nameStart, i - 1)
                        mappedExpression = mappedExpression .. mapName(sourceName, nameIsProperty)
                        nameStart = nil
                        nonNameStart = i
                    end
                elseif nameChar then
                    local lastChar = string.sub(expression, i - 1, i - 1)
                    nameIsProperty = lastChar == "."
                    nameStart = i
                    mappedExpression = mappedExpression .. string.sub(expression, nonNameStart, nameStart - (nameIsProperty and 2 or 1))
                end
            end
        end
        if nameStart then
            local sourceName = string.sub(expression, nameStart)
            mappedExpression = mappedExpression .. mapName(sourceName, nameIsProperty)
        else
            mappedExpression = mappedExpression .. string.sub(expression, nonNameStart)
        end
    ...
    local function debugBreak(activeThread, stackOffset, activeLine)
        ...
        while true do
            ...
            elseif string.sub(inp, 1, 6) == "thread" then
                local newThreadIdStr = string.match(inp, "^thread%s+(%d+)$")
            ...
            elseif inp == "stack" then
                backtrace(currentStack, frame)
            elseif string.sub(inp, 1, 5) == "frame" then
                local newFrameStr = string.match(inp, "^frame%s+(%d+)$")
            ...
            elseif inp == "globals" then
                ...
                Send.vars(globs)
            elseif string.sub(inp, 1, 5) == "break" then
                local cmd = string.match(inp, "^break%s+([a-z]+)")
                local file
                local line
                local breakpoint
                ...
                if cmd == "set" then
                    if file ~= nil and line ~= nil then
                        local condition = string.match(inp, "^break%s+[a-z]+%s+.-:%d+%s+(.+)")
            ...
            elseif string.sub(inp, 1, 4) == "eval" then
                local expression = string.match(inp, "^eval%s+(.+)$")
            ...
            elseif string.sub(inp, 1, 5) == "props" then
                local expression, kind, first, count = string.match(inp, "^props%s+(.-)%s*([a-z]+)%s*(%d*)%s*(%d*)$")
                if not expression then
                    Send.error("Bad expression")
            ...
            elseif string.sub(inp, 1, 4) == "exec" then
                local statement = string.match(inp, "^exec%s+(.+)$")
            ...
            elseif string.sub(inp, 1, 6) == "script" then
                local scriptFile = string.match(inp, "^script%s+(.+)$")
            ...
            elseif string.sub(inp, 1, 6) == "ignore" then
                local ignorePattern = string.match(inp, "^ignore%s+(.+)$")
    ...
    local function comparePaths(a, b)
        local isParentA = string.match(a, ("^%.%.[/" .. Path.separator) .. "]")
        local isParentB = string.match(b, ("^%.%.[/" .. Path.separator) .. "]")
        ...
        if a ~= string.sub(b, -aLen) then
            return false
        end
        if string.sub(a, 1, 1) == Path.separator then
            return true
        end
        local bSep = -(aLen + 1)
    ...
    local function debugHook(event, line)
        ...
        if breakAtDepth >= 0 then
            ...
            if stepBreak then
                ...
                if string.sub(topFrameSource.source, -#debuggerName) == debuggerName then
                    return
                end
                if topFrameSource.short_src and string.sub(topFrameSource.short_src, 1, #builtinFunctionPrefix) == builtinFunctionPrefix then
                    return
                end
                local source
                if ignorePatterns then
                    ...
                    for ____, pattern in ipairs(ignorePatterns) do
                        local match = string.match(source, pattern)
                        if match then
                            return
                        end
                    end
        ...
        if allowBreak and useXpcallInCoroutine() then
            ...
            local function debugFunc(...)
                ...
                local function wrappedFunc()
                    return originalFunc(table.unpack(args))
                end
...
function ____exports.runFile(filePath, breakImmediately, arg)
    ...
    local env = setmetatable(
        ...
        {
            __index = _G,
            __newindex = function(____, ____self, key, value)
                _G[key] = value
            end
        }
    )
...
local ____entry = require("lldebugger", ...)
return ____entry