    ...
    local function unpack(tbl)
        ...
        if not mt or not mt.__pairs then
            if option['sorter'] then
                ...
                tableSort(keys, function (a, b) return keymap[a] < keymap[b] end)
            end
        end
...
function m.viewString(str, quo)
    ...
    if quo == "'" then
        str = str:gsub('[\000-\008\011-\012\014-\031\127]', function (char) return ('\\%03d'):format(char:byte()) end)
        return quo .. str:gsub([=[['\r\n]]=], esc) .. quo
    elseif quo == '"' then
        str = str:gsub('[\000-\008\011-\012\014-\031\127]', function (char) return ('\\%03d'):format(char:byte()) end)
        return quo .. str:gsub([=[["\r\n]]=], esc) .. quo
...
function m.sortCallbackOfIndex(arr)
    ...
    return function (v) return - indexMap[v] end
end
...
function m.sortCallbackOfScore(datas, scores)
    ...
    return function (v) return map[v] end
end
...
function m.expandPath(path, env)
    ...
    elseif path:sub(1, 1) == '$' then
        path = path:gsub('%$([%w_]+)', function (name) return env and env[name] or getenv(name) or '' end)
        return path
    end
    return path
end
