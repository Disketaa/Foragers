...
function m.wait(timeout, onTimer)
    local _timeout = mathMax(mathFloor(timeout * 1000), 1)
...
function m.loop(timeout, onTimer)
    local _timeout = mathFloor(timeout * 1000)
    local t = setmetatable({
        ['_timeout'] = _timeout,
        ['_onTimer'] = onTimer,
    }, mt)
...
function m.timeJump(delta)
    fwFrame = fwFrame + mathFloor(delta * 1000)
end
