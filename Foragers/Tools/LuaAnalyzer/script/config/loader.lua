...
function m.loadLocalConfig(uri, filename)
    ...
    if firstChar == '{' then
        ...
        local suc, res = pcall(function () return assert(load(buf, '@' .. path, 't'))() end)
        if not suc then
            errorMessage(lang.script('CONFIG_LOAD_ERROR', res))
            scp:set('lastLocalConfig', res)
        end
...
function m.loadClientConfig(uri)
    local configs = proto.awaitRequest('workspace/configuration', {
        items = {
        	{ scopeUri = uri, section = 'Lua' },
        	{ scopeUri = uri, section = 'files.associations' },
        	{ scopeUri = uri, section = 'files.exclude' },
        	{ scopeUri = uri, section = 'editor.semanticHighlighting.enabled' },
        	{ scopeUri = uri, section = 'editor.acceptSuggestionOnEnter' },
        },
    })
