...
function m.reportMemoryCollect()
    ...
    lines[#lines+1] = ('        Total: %.3f(%.3f) MB'):format(totalMemAfter / 1000, totalMemBefore / 1000)
    for i = 0, #mems do
        lines[#lines+1] = ('        # %02d : %.3f MB'):format(i, mems[i] / 1000)
    end
...
function m.reportMemory()
    ...
    local lines = {}
    lines[#lines+1] = '    --------------- Memory ---------------'
    lines[#lines+1] = ('        Total: %.3f MB'):format(totalMem / 1000)
    for i = 0, #mems do
        lines[#lines+1] = ('        # %02d : %.3f MB'):format(i, mems[i] / 1000)
    end
    return table.concat(lines, '\n')
...
function m.report()
    local t = timer.loop(600, function ()
        local lines = {}
        lines[#lines+1] = ''
