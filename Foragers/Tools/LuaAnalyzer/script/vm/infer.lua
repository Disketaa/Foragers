...
local viewNodeSwitch;viewNodeSwitch = util.switch()
    ...
    : case 'string'
    : case 'integer'
    : call(function (source, _infer) return source.type end)
    : case 'number'
    : call(function (source, _infer) return source.type end)
    : case 'table'
    ...
    : case 'generic'
    : call(function (source, _infer, uri) return vm.getInfer(source.proto):view(uri) end)
    ...
    : call(function (source, _infer) return util.viewString(source[1], source[2]) end)
    : case 'doc.type.integer'
    : case 'doc.type.boolean'
    : call(function (source, _infer) return ('%q'):format(source[1]) end)
    : case 'doc.type.code'
    : call(function (source, _infer) return ('`%s`'):format(source[1]) end)
    ...
    : case 'doc.field.name'
    : call(function (source, _infer, uri) return vm.viewKey(source, uri) end)
