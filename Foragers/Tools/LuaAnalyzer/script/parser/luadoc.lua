...
Symbol              <-  ({} {
    ...
    Char16 = function (char) return string.char(tonumber(char, 16)) end,
    ...
    LongStringMark = function (back) return '[' .. back .. '[' end,
...
local function bindDocs(state)
    ...
    table.sort(sources, function (a, b) return a.start < b.start end)
    local binded
...
local function luadoc(state)
    ...
    table.sort(comments, function (a, b) return a.start < b.start end)
    ast.docs = {
        type   = 'doc',
        parent = ast,
        groups = {},
    ...
    if ast.state.pluginDocs then
        ...
        ---@param a unknown
        ---@param b unknown
        table.sort(ast.docs, function (a, b) return a.start < b.start end)
