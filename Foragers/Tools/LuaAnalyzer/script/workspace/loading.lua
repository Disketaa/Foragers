...
function mt:update()
    ...
    self._bar:setPercentage(self.read / self.max * 100)
end
    ...
    if files.isLua(uri) then
        ...
        self._stash[#self._stash+1] = function ()
            if files.getFile(uri) then
                ...
                log.info(('Preload file at: %s , size = %.3f KB'):format(uri, #content / 1024))
    ...
    elseif files.isDll(uri) then
        ...
        self._stash[#self._stash+1] = function ()
            if files.getFile(uri) then
                ...
                log.info(('Preload dll at: %s , size = %.3f KB'):format(uri, #content / 1024))
                --await.wait(function (waker)
                --    self._sets[#self._sets+1] = waker
                --end)
